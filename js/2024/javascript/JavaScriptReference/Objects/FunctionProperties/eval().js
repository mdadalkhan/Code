...
// // //Parameters // string // A string representing a JavaScript expression,
// // statement, or sequence of statements. The expression can include variables and
// // properties of existing objects. // Return value // The completion value of
// // evaluating the given code. If the completion value is empty, undefined is
// // returned.
...
// // If you use the eval function indirectly, by invoking it via a reference other
// // than eval, as of ECMAScript 5 it works in the global scope rather than the
...
// // Never use eval()! eval() is a dangerous function, which executes the code it's
// // passed with the privileges of the caller. If you run eval() with a string that
// // could be affected by a malicious party, you may end up running malicious code
// // on the user's machine with the permissions of your webpage / extension. More
// // importantly, a third-party code can see the scope in which eval() was invoked,
// // which can lead to possible attacks in ways to which the similar Function is
// // not susceptible.
...
// // Additionally, modern javascript interpreters convert javascript to machine
// // code. This means that any concept of variable naming gets obliterated. Thus,
// // any use of eval() will force the browser to do long expensive variable name
// // lookups to figure out where the variable exists in the machine code and set
// // its value. Additionally, new things can be introduced to that variable through
...
// // window.Function(). See this example of how to convert code using a dangerous
// // eval() to using Function(), see below.
...
// // Comparing the two code snippets above, the two code snippets might seem to
// // work the same way, but think again: the eval() one is a great deal slower.
// // Notice c: new Date() in the evaluated object. In the function without the
// // eval(), the object is being evaluated in the global scope, so it is safe for
// // the browser to assume that Date refers to window.Date() instead of a local
// // variable called Date. But, in the code using eval(), the browser cannot assume
// // this since what if your code looked like the following:
...
// // minified. Function call overhead is minimal, making the far smaller code size
// // well worth the benefit Function() more easily allows your code to utilize the
// // performance buttering "use strict"; The code does not use eval(), making it
// // orders of magnitude faster than otherwise. Lastly, let's examine minification.
// // With using Function() as shown above, you can minify the code string passed to
// // runCodeWithDateFunction() far more efficiently because the function arguments
// // names can be minified too as seen in the minified code below.
...
// Accessing member properties You should not use eval() to convert property
// names into properties. Consider the following example where the property of
// the object to be accessed is not known until the code is executed. This can be
// done with eval():
...
// Use functions instead of evaluating snippets of code JavaScript has
// first-class functions, which means you can pass functions as arguments to
// other APIs, store them in variables and objects' properties, and so on. Many
// DOM APIs are designed with this in mind, so you can (and should) write:
...
// Parsing JSON (converting strings to JavaScript objects) If the string you're
// calling eval() on contains data (for example, an array: "[1, 2, 3]"), as
// opposed to code, you should consider switching to JSON, which allows the
// string to use a subset of JavaScript syntax to represent data.
...
// Pass data instead of code For example, an extension designed to scrape
// contents of web-pages could have the scraping rules defined in XPath instead
// of JavaScript code.




// Examples Using eval In the following code, both of the statements containing
// eval() return 42. The first evaluates the string "x + y + 1"; the second
// evaluates the string "42".
...
// Last expression is evaluated eval() returns the value of the last expression
// evaluated.
