// Object.create() The Object.create() method creates a new object, using an
...
// propertiesObject Optional If specified and not undefined, an object whose
// enumerable own properties (that is, those properties defined upon itself
// and not enumerable properties along its prototype chain) specify property
...
// Custom and Null objects A new object created from a completely custom object
// (especially one created from the null object, which is basically a custom
// object with NO members) can behave in unexpected ways. This is especially true
// when debugging, since common object-property converting/detecting utility
// functions may generate errors, or lose information (especially if using silent
// error-traps that ignore errors). For example, here are two objects:
...
// Testing just a few of the many most basic built-in functions shows the
// magnitude of the problem more clearly:
...
// Not such simple results: (especially if silent error-trapping had hidden
// the error messages)
...
// property-adding code-branches actually get executed at runtime as depends
// on inputs and/or random-variables. Then again, the actual iteration order
// is not guaranteed no matter what the order members are added.
...
// Adding the missing object-method directly to new object's "prototype" does
// not work either, since the new object does not have a real prototype (which
// is really the cause of ALL these problems) and one cannot be directly
// added:
...
// Adding the missing object-method by using the standard-object as new
// object's prototype does not work either:
...
// Some OK solutions Again, adding the missing object-method directly from the
// standard-object does NOT work. However, adding the generic method directly,
// DOES:
...
// which the prototype has been chosen but doesn't take the second argument
// into account.
...
// Object.assign() copies properties from the OtherSuperClass prototype to the
// MyClass prototype, making them available to all instances of MyClass.
// Object.assign() was introduced with ES2015 and can be polyfilled. If
// support for older browsers is necessary, jQuery.extend() or _.assign() can
// be used.
